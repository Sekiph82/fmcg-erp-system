...
const BYPASS_PREFIX_REDIRECT = new Set([
  ...
  "/dashboard/secondary-sales","/dashboard/secondary-sales/analysis","/dashboard/secondary-sales/inventory","/dashboard/secondary-sales/upload",
...
function classifyTarget(targetRoute) {
  ...
  // 1b. Template literal routes like /dashboard/users/${r.id} — check for [id]/page.tsx
  if (targetRoute.includes("${")) {
    const basePath = targetRoute.split("${")[0].replace(/\/$/, "");
    const idPageAbs = path.join(FRONTEND, "app", ...basePath.split("/").filter(Boolean), "[id]", "page.tsx");
    if (fileExists(idPageAbs) && !isRedirectStub(idPageAbs)) {
      return { status: "WORKING", reason: "dynamic_id_page_exists" };
    }
