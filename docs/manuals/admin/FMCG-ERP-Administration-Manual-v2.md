...
**Current mode:** Simulation (no live KRA calls)
**Route:** `/dashboard/finance/etims`

The eTIMS connector is architecturally complete. The SimulationETIMSConnector is active by default — submissions are processed locally with fake ACCEPTED responses. `production_execution_allowed = false`.

**10 submission statuses:** DRAFT, READY, PENDING, SUBMITTED, RETRY_PENDING, ACCEPTED, REJECTED, FAILED, ERROR, CANCELLED

**To enable live mode (do not do this without authorization):**
1. Select a KRA-approved eTIMS provider
2. Register device at KRA eTIMS developer portal
3. Set `ETIMS_PROVIDER=http`, `ETIMS_API_URL`, `ETIMS_API_KEY`, `ETIMS_PIN`, `ETIMS_BRANCH_CODE` in `.env.production`
4. Obtain accountant approval for GL posting gate rules (TASK-005.1D)

**GL posting gate:** Finance GL journal posting does NOT currently check eTIMS status — invoices can be posted regardless of REJECTED/ERROR status. This gate is planned but blocked on accountant confirmation.

**What admins can do in simulation:**
- View all submissions and statuses (Finance → eTIMS tab)
- Retry / Cancel / Poll submissions (routed to simulator)
- Check provider health panel
- View debug payloads per submission
...
## 15. Security Hardening — Post ERP Updates (v2.1)

### Management User Seeding (No Hardcoded Secrets)

Management superuser accounts are created at startup via environment variables:
- `MANAGEMENT_USER_EMAIL`, `MANAGEMENT_USER_USERNAME`, `MANAGEMENT_USER_PASSWORD`, `MANAGEMENT_USER_FULL_NAME`
- If `MANAGEMENT_USER_PASSWORD` is not set, seed exits without creating the account
- **Force password change** is set on first login — initial seed password must be changed before accessing any module

**Before go-live:** Confirm the management user's initial password has been changed. Check Admin → Users → user record → `force_password_change = false` after first login.

### PyJWT Token Security

The system uses PyJWT (not `python-jose`). Do not add `python-jose` to requirements.

| Config | Value | Notes |
|---|---|---|
| Algorithm | HS256 | Symmetric — `SECRET_KEY` must be secret |
| `SECRET_KEY` | Must be 32+ chars random | Generate: `openssl rand -hex 32` |
| `ACCESS_TOKEN_EXPIRE_MINUTES` | Default 30 | Increase for less friction; decrease for higher security |
| Token rejection | Expired tokens return HTTP 401 | No grace period |

**If `SECRET_KEY` is compromised:** Rotate it immediately. All active sessions are invalidated. Users must log in again.

### GS1 Label Generator — Auth-Gated

All 38 GS1 label endpoints require authentication (`require_auth`). Unauthenticated requests return HTTP 401. The frontend GS1 page at `/dashboard/compliance/gs1` requires an active login session.

Do not bypass or remove these guards. GS1 barcodes on product packaging must only be generated by authorized users.

### Alembic Migration Advisory Lock

Migrations use `pg_advisory_lock(20260517)` to prevent concurrent migration runs across replicas.

**If startup hangs on migrations:** Check if another replica holds the lock:
```sql
SELECT pid, query, state FROM pg_stat_activity WHERE query LIKE '%advisory%';
```

If a dead replica holds the lock, release it:
```sql
SELECT pg_advisory_unlock(20260517);
```

Then restart the pending replica.

### Blocked Live Integrations — Do Not Enable Without Prerequisites

| Integration | Status | Required before enabling |
|---|---|---|
| eTIMS (KRA) | Simulation | Provider selection + KRA sandbox creds + accountant GL gate approval |
| M-Pesa (Daraja) | Simulation | Safaricom credentials (Consumer Key, Secret, Shortcode, Passkey, Callback URL) |
| WhatsApp (Meta) | Demo mode | Meta Cloud API token + Phone Number ID in DB config |
| AI LLM features | Mock | Anthropic/OpenAI/Gemini API key |
| SMTP OTP | Dev delivery | Confirm `OTP_DEV_DELIVERY_MODE=false` in production |

---

*End of Administration Manual v2.1*
