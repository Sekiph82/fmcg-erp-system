...
import { kanbanApi, KBAIRec, KBAIAgentType, KBAIRecStatus } from "@/lib/kanban";

const AGENT_META: Record<KBAIAgentType, { label: string; desc: string; color: string }> = {
  workflow_optimizer: {
    label: "Workflow Optimizer",
    desc: "Detects WIP limit violations and column bottlenecks where >60% of cards are stuck in one column.",
    color: "bg-blue-50 border-blue-200",
  },
  task_prioritizer: {
    label: "Task Prioritizer",
    desc: "Flags overdue active cards and unassigned critical-priority cards that need immediate action.",
    color: "bg-orange-50 border-orange-200",
  },
};

const STATUS_COLOR: Record<KBAIRecStatus, string> = {
...
export default function KanbanAIPage() {
  const [recs, setRecs] = useState<KBAIRec[]>([]);
  const [loading, setLoading] = useState(true);
  const [running, setRunning] = useState<KBAIAgentType | null>(null);

  const load = () => kanbanApi.listAIRecs().then(setRecs).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);

  const run = async (agent: KBAIAgentType) => {
    setRunning(agent);
    try {
      if (agent === "workflow_optimizer") await kanbanApi.runWorkflowOptimizer();
      else await kanbanApi.runTaskPrioritizer();
      load();
    } finally { setRunning(null); }
  };

  const ack = async (id: string, status: KBAIRecStatus) => {
    await kanbanApi.ackAIRec(id, { status });
    load();
  };

  return (
    <div className="p-6 space-y-6">
      <h1 className="text-xl font-bold text-gray-900">Kanban AI Insights</h1>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {(Object.entries(AGENT_META) as [KBAIAgentType, typeof AGENT_META[KBAIAgentType]][]).map(([type, meta]) => (
