...
import { calendarApi, CAIRec, CAIAgentType, CAIRecStatus } from "@/lib/calendar";

const AGENT_META: Record<CAIAgentType, { label: string; desc: string; color: string }> = {
  schedule_optimizer: {
    label: "Schedule Optimizer",
    desc: "Identifies under-utilized resources this week and suggests optimal allocation to pending tasks or production activities.",
    color: "bg-blue-50 border-blue-200",
  },
  conflict_resolver: {
    label: "Conflict Resolver",
    desc: "Detects overlapping bookings and back-to-back schedules with insufficient buffer time, and recommends resolution steps.",
    color: "bg-orange-50 border-orange-200",
  },
};

const STATUS_COLOR: Record<CAIRecStatus, string> = {
...
export default function CalendarAIPage() {
  const [recs, setRecs] = useState<CAIRec[]>([]);
  const [loading, setLoading] = useState(true);
  const [running, setRunning] = useState<CAIAgentType | null>(null);

  const load = () => calendarApi.listAIRecs().then(setRecs).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);

  const run = async (agent: CAIAgentType) => {
    setRunning(agent);
    try {
      if (agent === "schedule_optimizer") await calendarApi.runScheduleOptimizer();
      else await calendarApi.runConflictResolver();
      load();
    } finally { setRunning(null); }
  };

  const ack = async (id: string, status: CAIRecStatus) => {
    await calendarApi.ackAIRec(id, { status });
    load();
  };

  return (
    <div className="p-6 space-y-6">
      <h1 className="text-xl font-bold text-gray-900">Calendar AI Insights</h1>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {(Object.entries(AGENT_META) as [CAIAgentType, typeof AGENT_META[CAIAgentType]][]).map(([type, meta]) => (
