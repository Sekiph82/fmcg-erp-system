...
const MANIFEST_PATH = path.resolve(
  __dirname,
  "../../docs/user-manual/screenshots/marketing-ui-screenshot-manifest.json"
);
...
const INDEX_PATH = path.resolve(
  __dirname,
  "../../docs/user-manual/screenshots/marketing-ui-screenshots-index.json"
);
...
function filterItems(items: ManifestItem[]): ManifestItem[] {
  const page = process.env.MARKETING_PAGE;
  const ids = process.env.MARKETING_IDS;
  const missingOnly = process.env.MARKETING_MISSING_ONLY === "1";
  const priority = process.env.MARKETING_PRIORITY;

  let filtered = items;

  if (page) {
    filtered = filtered.filter((item) => item.path.includes(`/marketing/${page}/`));
  }
  if (ids) {
