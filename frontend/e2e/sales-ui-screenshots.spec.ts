/**
 * Sales & Distribution UI Screenshot Capture
 *
 * Reads sales-ui-screenshot-manifest.json, captures each item,
 * writes sales-ui-screenshots-index.json after every capture.
 *
 * Env filters:
 *   SALES_PAGE=sales|all
 *   SALES_IDS=id1,id2,id3
 *   SALES_REQUIRED_ONLY=true
 *   SALES_MISSING_ONLY=true
 *   SALES_PRIORITY=1|2|3
 *
 * Run (app must be running on localhost:3000):
 *   E2E_SKIP_WEBSERVER=1 npx playwright test e2e/sales-ui-screenshots.spec.ts --project=chromium --workers=1
 */
...
const MANIFEST_FILE = path.join(REPO_ROOT, "docs/user-manual/screenshots/sales-ui-screenshot-manifest.json");
const INDEX_FILE    = path.join(REPO_ROOT, "docs/user-manual/screenshots/sales-ui-screenshots-index.json");
...
test.describe("Sales & Distribution UI Screenshots", () => {
  ...
  for (const item of MANIFEST) {
    test(`capture: ${item.id}`, async ({ page }) => {
      const PAGE_FILTER   = (process.env.SALES_PAGE   || "all").toLowerCase();
      const IDS_FILTER    = (process.env.SALES_IDS    || "").split(",").filter(Boolean);
      const REQUIRED_ONLY = process.env.SALES_REQUIRED_ONLY === "true";
      const MISSING_ONLY  = process.env.SALES_MISSING_ONLY  === "true";
      const MAX_PRIORITY  = parseInt(process.env.SALES_PRIORITY || "3");
