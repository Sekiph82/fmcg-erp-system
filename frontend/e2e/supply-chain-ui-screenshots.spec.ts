/**
 * Supply Chain UI Screenshot Capture
 *
 * Reads supply-chain-ui-screenshot-manifest.json, captures each item,
 * writes supply-chain-ui-screenshots-index.json after every capture.
 *
 * Env filters:
 *   SUPPLY_CHAIN_PAGE=procurement|suppliers|inventory|movements|cycle-count|shelf-life|warehouses|wms|logistics|all
 *   SUPPLY_CHAIN_IDS=id1,id2,id3
 *   SUPPLY_CHAIN_REQUIRED_ONLY=true
 *   SUPPLY_CHAIN_MISSING_ONLY=true
 *   SUPPLY_CHAIN_PRIORITY=1|2|3
 *
 * Run (app must be running on localhost:3000):
 *   E2E_SKIP_WEBSERVER=1 npx playwright test e2e/supply-chain-ui-screenshots.spec.ts --project=chromium --workers=1
 */
...
const MANIFEST_FILE = path.join(REPO_ROOT, "docs/user-manual/screenshots/supply-chain-ui-screenshot-manifest.json");
const INDEX_FILE    = path.join(REPO_ROOT, "docs/user-manual/screenshots/supply-chain-ui-screenshots-index.json");
...
test.describe("Supply Chain UI Screenshots", () => {
  let indexMap: Map<string, IndexEntry>;

  test.beforeAll(() => {
    indexMap = loadIndex();

    // Pre-populate parent-captured items in index
  ...
  for (const item of MANIFEST) {
    test(`capture: ${item.id}`, async ({ page }) => {
      const PAGE_FILTER     = (process.env.SUPPLY_CHAIN_PAGE     || "all").toLowerCase();
      const IDS_FILTER      = (process.env.SUPPLY_CHAIN_IDS      || "").split(",").filter(Boolean);
      const REQUIRED_ONLY   = process.env.SUPPLY_CHAIN_REQUIRED_ONLY === "true";
      const MISSING_ONLY    = process.env.SUPPLY_CHAIN_MISSING_ONLY  === "true";
      const MAX_PRIORITY    = parseInt(process.env.SUPPLY_CHAIN_PRIORITY || "3");

      // Skip pre-classified items and items with no output
