...
const MANIFEST_PATH = path.resolve(
  __dirname,
  "../../docs/user-manual/screenshots/pos-ui-screenshot-manifest.json"
);
...
const INDEX_PATH = path.resolve(
  __dirname,
  "../../docs/user-manual/screenshots/pos-ui-screenshots-index.json"
);
...
function filterItems(items: ManifestItem[]): ManifestItem[] {
  const ids = process.env.POS_IDS;
  const missingOnly = process.env.POS_MISSING_ONLY === "1";
  const priority = process.env.POS_PRIORITY;
