/**
 * Manufacturing UI Screenshot Capture
 *
 * Reads manufacturing-ui-screenshot-manifest.json, captures each item,
 * writes manufacturing-ui-screenshots-index.json after every capture.
 *
 * Env filters:
 *   MANUFACTURING_PAGE=production|planning|npd|bom|recipes|quality|compliance|shop-floor|all
 *   MANUFACTURING_IDS=id1,id2,id3
 *   MANUFACTURING_REQUIRED_ONLY=true
 *   MANUFACTURING_MISSING_ONLY=true
 *   MANUFACTURING_PRIORITY=1|2|3
 *
...
const MANIFEST_FILE = path.join(REPO_ROOT, "docs/user-manual/screenshots/manufacturing-ui-screenshot-manifest.json");
const INDEX_FILE    = path.join(REPO_ROOT, "docs/user-manual/screenshots/manufacturing-ui-screenshots-index.json");
...
test.describe("Manufacturing UI Screenshots", () => {
  ...
  for (const item of MANIFEST) {
    test(`capture: ${item.id}`, async ({ page }) => {
      const PAGE_FILTER     = (process.env.MANUFACTURING_PAGE     || "all").toLowerCase();
      const IDS_FILTER      = (process.env.MANUFACTURING_IDS      || "").split(",").filter(Boolean);
      const REQUIRED_ONLY   = process.env.MANUFACTURING_REQUIRED_ONLY === "true";
      const MISSING_ONLY    = process.env.MANUFACTURING_MISSING_ONLY  === "true";
      const MAX_PRIORITY    = parseInt(process.env.MANUFACTURING_PRIORITY || "3");
