...
class BRBankAccount(Base, TimestampMixin):
    ...
    account_type             = Column(Enum(BRAccountType, native_enum=False), nullable=False, default=BRAccountType.CURRENT)
...
class BRStatement(Base, TimestampMixin):
    ...
    import_source      = Column(Enum(BRImportSource, native_enum=False), nullable=False, default=BRImportSource.MANUAL)
    import_file_name   = Column(String(255), nullable=True)
    status             = Column(Enum(BRStatementStatus, native_enum=False), nullable=False, default=BRStatementStatus.DRAFT)
...
class BRStatementLine(Base, TimestampMixin):
    ...
    match_status         = Column(Enum(BRLineMatchStatus, native_enum=False), nullable=False, default=BRLineMatchStatus.UNMATCHED)
...
class BRMatch(Base, TimestampMixin):
    ...
    erp_transaction_type   = Column(Enum(BRERPTransactionType, native_enum=False), nullable=False)
    ...
    match_method           = Column(Enum(BRMatchMethod, native_enum=False), nullable=False, default=BRMatchMethod.MANUAL)
...
class BRRule(Base, TimestampMixin):
    ...
    condition_type          = Column(Enum(BRRuleConditionType, native_enum=False), nullable=False)
    condition_value         = Column(String(500), nullable=False)
    target_transaction_type = Column(Enum(BRERPTransactionType, native_enum=False), nullable=False)
...
class BRAdjustment(Base, TimestampMixin):
    ...
    adjustment_type    = Column(Enum(BRAdjustmentType, native_enum=False), nullable=False)
    amount             = Column(Numeric(14, 4), nullable=False)
...
class BRAIRecommendation(Base, TimestampMixin):
    ...
    statement_line_id  = Column(UUID(as_uuid=True), ForeignKey("br_statement_lines.id", ondelete="SET NULL"), nullable=True)
    agent_type         = Column(Enum(BRAIAgentType, native_enum=False), nullable=False)
    ...
    confidence_score   = Column(Numeric(5, 4), nullable=True)
    status             = Column(Enum(BRAIRecStatus, native_enum=False), nullable=False, default=BRAIRecStatus.PENDING)
