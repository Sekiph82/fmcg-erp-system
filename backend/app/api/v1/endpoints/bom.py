...
async def create_bom(body: BOMCreate, db: AsyncSession = Depends(get_db)):
    bom = await svc.create_bom(db, body, _uid())
    await db.commit()
...
async def update_bom(bom_id: UUID, body: BOMUpdate, db: AsyncSession = Depends(get_db)):
    bom = await svc.get_bom(db, bom_id)
    if not bom:
        raise HTTPException(404, "BOM not found")
...
async def clone_bom(
    ...
    if not bom:
        raise HTTPException(404, "BOM not found")
    cloned = await svc.clone_bom(db, bom, new_version, _uid())
    await db.commit()
...
async def advance_lifecycle(bom_id: UUID, db: AsyncSession = Depends(get_db)):
    bom = await svc.get_bom(db, bom_id)
    if not bom:
        raise HTTPException(404, "BOM not found")
    try:
        updated = await svc.advance_lifecycle(db, bom, _uid())
        await db.commit()
        await db.refresh(updated)
        return updated
...
async def archive_bom(bom_id: UUID, db: AsyncSession = Depends(get_db)):
    bom = await svc.get_bom(db, bom_id)
    if not bom:
        raise HTTPException(404, "BOM not found")
...
async def add_line(bom_id: UUID, body: BOMLineCreate, db: AsyncSession = Depends(get_db)):
...
async def list_lines(bom_id: UUID, db: AsyncSession = Depends(get_db)):
    r = await db.execute(
...
async def update_line(line_id: UUID, body: BOMLineUpdate, db: AsyncSession = Depends(get_db)):
...
async def scale_bom(bom_id: UUID, body: ScalingRequest, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_costing(bom_id: UUID, db: AsyncSession = Depends(get_db)):
    try:
...
async def compare_versions(bom_id: UUID, other_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def get_compliance_summary(bom_id: UUID, db: AsyncSession = Depends(get_db)):
    try:
...
async def run_ai(bom_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def list_ai_recs(bom_id: UUID, db: AsyncSession = Depends(get_db)):
    return await svc.list_ai_recs(db, bom_id)


@router.post("/ai-recommendations/{rec_id}/action", response_model=BOMAIRecOut)
async def action_ai_rec(rec_id: UUID, body: BOMAIRecAction, db: AsyncSession = Depends(get_db)):
    try:
        rec = await svc.action_ai_rec(db, rec_id, body.status, _uid())
        await db.commit()
...
async def add_yield_config(bom_id: UUID, body: YieldConfigCreate, db: AsyncSession = Depends(get_db)):
...
async def list_yield_configs(bom_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_substitute_group(body: SubstGroupCreate, db: AsyncSession = Depends(get_db)):
...
async def list_substitute_groups(db: AsyncSession = Depends(get_db)):
    return await svc.list_substitute_groups(db)


@router.post("/substitutes", response_model=SubstituteOut, status_code=201)
async def add_substitute(body: SubstituteCreate, db: AsyncSession = Depends(get_db)):
...
async def create_conversion_profile(body: ConversionProfileCreate, db: AsyncSession = Depends(get_db)):
...
async def list_conversion_profiles(db: AsyncSession = Depends(get_db)):
