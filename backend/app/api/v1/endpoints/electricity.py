"""
Electricity Management API
──────────────────────────
Prefix: /electricity (registered in router.py)

Analytics (read-only, ELECTRICITY-scoped):
  GET  /electricity/kpis                  – plant KPI block
  GET  /electricity/trend/daily           – daily time-series
  GET  /electricity/trend/shift           – shift-based analysis
  GET  /electricity/breakdown             – GROUP BY dept/line/machine/building/shift

Transactions (ELECTRICITY records):
  GET  /electricity/transactions          – filtered list (delegates to tx CRUD)
  GET  /electricity/transactions/export/csv
  POST /electricity/transactions          – create one electricity record
  PATCH /electricity/transactions/{id}    – update
  DELETE /electricity/transactions/{id}   – delete

Tariffs (electricity tariffs for UI dropdown):
  GET  /electricity/tariffs               – active tariffs for ELECTRICITY type
"""
from __future__ import annotations

import csv
import io
import logging
import uuid
from datetime import date
from typing import List, Optional

from fastapi import APIRouter, Depends, HTTPException, Query
from fastapi.responses import StreamingResponse
from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from app.core.deps import require_permission
from app.db.session import get_db
from app.crud import utility_management as crud
from app.models.utility_management import UtilityTariff, UtilityType, SourceMethod, DataQuality, TxReferenceType
from app.schemas.electricity import (
    ElectricityBreakdown,
    ElectricityKPIs,
    ElectricityShiftPoint,
    ElectricityTariffRead,
    ElectricityTrendPoint,
)
from app.schemas.utility_management import (
    UtilityTransactionCreate,
    UtilityTransactionRead,
    UtilityTransactionUpdate,
)
from app.services.electricity_service import (
    get_breakdown,
    get_daily_trend,
    get_kpis,
    get_shift_analysis,
)
from app.services.utility_transaction_service import (
    create_transaction as svc_create_transaction,
    enrich_transaction,
)

logger = logging.getLogger(__name__)
router = APIRouter()

_PERM = "utility_management"


# ── Shared date-range params ──────────────────────────────────────────────────

def _date_range(
    date_from: Optional[date] = Query(None),
    date_to:   Optional[date] = Query(None),
    department:    Optional[str] = Query(None),
    building_area: Optional[str] = Query(None),
    line_id:       Optional[str] = Query(None),
    machine_id:    Optional[str] = Query(None),
    shift_id:      Optional[str] = Query(None),
):
    return dict(
        date_from=date_from, date_to=date_to,
        department=department, building_area=building_area,
        line_id=line_id, machine_id=machine_id, shift_id=shift_id,
    )


# ── Analytics ─────────────────────────────────────────────────────────────────

@router.get("/kpis", response_model=ElectricityKPIs)
async def electricity_kpis(
    scope: dict = Depends(_date_range),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    """Plant-wide (or scoped) electricity KPI block."""
    return await get_kpis(db, **scope)


@router.get("/trend/daily", response_model=List[ElectricityTrendPoint])
async def electricity_daily_trend(
    scope: dict = Depends(_date_range),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    """Daily kWh + cost time-series for trend charts."""
    return await get_daily_trend(db, **scope)


@router.get("/trend/shift", response_model=List[ElectricityShiftPoint])
async def electricity_shift_trend(
    date_from:  Optional[date] = Query(None),
    date_to:    Optional[date] = Query(None),
    department: Optional[str]  = Query(None),
    line_id:    Optional[str]  = Query(None),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    """Per-shift kWh aggregation (day/night/afternoon analysis)."""
    return await get_shift_analysis(
        db, date_from=date_from, date_to=date_to,
        department=department, line_id=line_id,
    )


@router.get("/breakdown", response_model=ElectricityBreakdown)
async def electricity_breakdown(
    dimension: str = Query("department", description="department | line | machine | building_area | shift"),
    scope: dict = Depends(_date_range),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    """
    Consumption breakdown grouped by a single dimension.
    dimension ∈ {department, line, machine, building_area, shift}
    """
    return await get_breakdown(db, dimension=dimension, **scope)


# ── Tariffs ───────────────────────────────────────────────────────────────────

@router.get("/tariffs", response_model=List[ElectricityTariffRead])
async def list_electricity_tariffs(
    is_active: Optional[bool] = Query(None),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    """List tariffs for ELECTRICITY — used for the transaction form dropdown."""
    q = select(UtilityTariff).where(UtilityTariff.utility_type == UtilityType.ELECTRICITY)
    if is_active is not None:
        q = q.where(UtilityTariff.is_active == is_active)
    q = q.order_by(UtilityTariff.tariff_code)
    rows = list((await db.execute(q)).scalars().all())
    return [ElectricityTariffRead.model_validate(r) for r in rows]


# ── Transactions ──────────────────────────────────────────────────────────────

@router.get("/transactions/export/csv")
async def export_electricity_csv(
    date_from:    Optional[date] = Query(None),
    date_to:      Optional[date] = Query(None),
    department:    Optional[str] = Query(None),
    building_area: Optional[str] = Query(None),
    line_id:       Optional[str] = Query(None),
    machine_id:    Optional[str] = Query(None),
    shift_id:      Optional[str] = Query(None),
    source_meter_id: Optional[uuid.UUID] = Query(None),
    source_asset_id: Optional[uuid.UUID] = Query(None),
    is_anomaly:    Optional[bool] = Query(None),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    rows = await crud.list_transactions(
        db,
        utility_type=UtilityType.ELECTRICITY,
        date_from=date_from, date_to=date_to,
        department=department, building_area=building_area,
        line_id=line_id, machine_id=machine_id, shift_id=shift_id,
        source_meter_id=source_meter_id, source_asset_id=source_asset_id,
        is_anomaly=is_anomaly,
        limit=50000,
    )

    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow([
        "transaction_no", "transaction_date", "quantity_kwh", "unit",
        "total_cost", "currency", "cost_rate",
        "department", "building_area", "production_line", "machine",
        "shift", "batch_no", "source_method", "quality",
        "estimated", "anomaly", "anomaly_note",
        "device_code", "asset_no", "tariff_code", "notes",
    ])
    for tx in rows:
        w.writerow([
            tx.transaction_no, tx.transaction_date,
            tx.quantity, tx.unit_of_measure,
            tx.total_cost, tx.currency_code, tx.cost_rate,
            tx.department, tx.building_area, tx.production_line, tx.machine_ref,
            tx.shift_ref, tx.batch_no, tx.source_method.value, tx.quality.value,
            tx.is_estimated, tx.is_anomaly, tx.anomaly_note,
            tx.device.device_code if tx.device else "",
            tx.asset.asset_no if tx.asset else "",
            tx.tariff.tariff_code if tx.tariff else "",
            tx.notes,
        ])

    filename = f"electricity-transactions-{date.today().isoformat()}.csv"
    return StreamingResponse(
        iter([buf.getvalue()]),
        media_type="text/csv",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
    )


@router.get("/transactions", response_model=List[UtilityTransactionRead])
async def list_electricity_transactions(
    date_from:    Optional[date] = Query(None),
    date_to:      Optional[date] = Query(None),
    department:    Optional[str] = Query(None),
    building_area: Optional[str] = Query(None),
    line_id:       Optional[str] = Query(None),
    machine_id:    Optional[str] = Query(None),
    shift_id:      Optional[str] = Query(None),
    source_meter_id: Optional[uuid.UUID] = Query(None),
    source_asset_id: Optional[uuid.UUID] = Query(None),
    batch_id:      Optional[str] = Query(None),
    source_method: Optional[SourceMethod] = Query(None),
    quality:       Optional[DataQuality] = Query(None),
    is_anomaly:    Optional[bool] = Query(None),
    is_estimated:  Optional[bool] = Query(None),
    search:        Optional[str] = Query(None),
    skip:  int = Query(0, ge=0),
    limit: int = Query(200, le=2000),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    """List electricity transactions with full filter set."""
    txs = await crud.list_transactions(
        db,
        utility_type=UtilityType.ELECTRICITY,
        date_from=date_from, date_to=date_to,
        department=department, building_area=building_area,
        line_id=line_id, machine_id=machine_id, shift_id=shift_id,
        source_meter_id=source_meter_id, source_asset_id=source_asset_id,
        batch_id=batch_id, source_method=source_method,
        is_anomaly=is_anomaly, is_estimated=is_estimated,
        search=search, skip=skip, limit=limit,
    )
    return [enrich_transaction(tx) for tx in txs]


@router.get("/transactions/{tx_id}", response_model=UtilityTransactionRead)
async def get_electricity_transaction(
    tx_id: uuid.UUID,
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "view")),
):
    tx = await crud.get_transaction(db, tx_id)
    if not tx or tx.utility_type != UtilityType.ELECTRICITY:
        raise HTTPException(404, "Electricity transaction not found")
    return enrich_transaction(tx)


@router.post("/transactions", response_model=UtilityTransactionRead, status_code=201)
async def create_electricity_transaction(
    data: UtilityTransactionCreate,
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "create")),
):
    """
    Create a new electricity consumption record.
    Forces utility_type = ELECTRICITY regardless of payload value.
    """
    # Force utility type
    data = data.model_copy(update={"utility_type": UtilityType.ELECTRICITY})
    tx = await svc_create_transaction(db, data)
    await db.commit()
    return tx


@router.patch("/transactions/{tx_id}", response_model=UtilityTransactionRead)
async def update_electricity_transaction(
    tx_id: uuid.UUID,
    data: UtilityTransactionUpdate,
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "edit")),
):
    tx = await crud.get_transaction(db, tx_id)
    if not tx or tx.utility_type != UtilityType.ELECTRICITY:
        raise HTTPException(404, "Electricity transaction not found")
    tx = await crud.update_transaction(db, tx, data)
    await db.commit()
    obj = await crud.get_transaction(db, tx_id)
    return enrich_transaction(obj)


@router.delete("/transactions/{tx_id}", status_code=204)
async def delete_electricity_transaction(
    tx_id: uuid.UUID,
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_permission(_PERM, "delete")),
):
    tx = await crud.get_transaction(db, tx_id)
    if not tx or tx.utility_type != UtilityType.ELECTRICITY:
        raise HTTPException(404, "Electricity transaction not found")
    await crud.delete_transaction(db, tx)
    await db.commit()
