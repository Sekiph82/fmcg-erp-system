...
PERMISSIONS = [
    ...
    # NPD / formula governance
    ("npd", "view", "View NPD", "View NPD projects, stage gates, pilot batches, and dashboard summaries", True),
    ("npd", "create", "Create NPD", "Create new product development projects", False),
    ("npd", "edit", "Edit NPD", "Edit NPD projects and readiness checklists", False),
    ("npd", "approve", "Approve NPD Gates", "Approve NPD stage gates", False),
    ("npd", "advance", "Advance NPD Stage", "Advance NPD projects through governed stage gates", False),
    ("npd", "pilot", "Manage NPD Pilot Batches", "Create NPD pilot batch records", False),
    ("npd", "export", "Export NPD", "Export NPD project and governance reports", False),
    ("bom", "view", "View BOM", "View advanced BOM/formula records and governance summaries", True),
    ("bom", "create", "Create BOM", "Create advanced BOM/formula records and supporting master data", False),
    ("bom", "edit", "Edit BOM", "Edit advanced BOM/formula headers, lines, yields, and lifecycle drafts", False),
    ("bom", "delete", "Delete BOM", "Delete BOM lines and other removable formula records", False),
    ("bom", "approve", "Approve BOM", "Approve formula lifecycle transitions", False),
    ("bom", "release", "Release BOM", "Release approved formulas for operational use", False),
    ("bom", "archive", "Archive BOM", "Archive formula records", False),
    ("bom", "cost", "Cost BOM", "Run BOM costing and formula cost reports", False),
    ("bom", "ai", "Run BOM AI", "Run or action BOM AI recommendations", False),
    ("bom", "export", "Export BOM", "Export BOM/formula reports", False),
    ("recipe", "view", "View Recipes", "View recipe headers, ingredients, and process parameters", True),
    ("recipe", "create", "Create Recipes", "Create recipe records and duplicates", False),
    ("recipe", "edit", "Edit Recipes", "Edit draft recipes, items, and process parameters", False),
    ("recipe", "delete", "Delete Recipes", "Delete draft recipe records or lines", False),
    ("recipe", "approve", "Approve Recipes", "Approve draft recipes for use", False),
    ("recipe", "obsolete", "Obsolete Recipes", "Mark recipes obsolete", False),
    ("recipe", "export", "Export Recipes", "Export recipe reports", False),
    ...
    # Consumer complaints / recall linkage
    ("consumer_complaints", "view",        "View Consumer Complaints",        "View consumer complaint records and complaint KPIs", True),
    ("consumer_complaints", "create",      "Create Consumer Complaints",      "Log consumer complaints received from customers or consumers", False),
    ("consumer_complaints", "edit",        "Edit Consumer Complaints",        "Update investigation details and complaint assignments", False),
    ("consumer_complaints", "delete",      "Delete Consumer Complaints",      "Delete consumer complaint records", False),
    ("consumer_complaints", "approve",     "Approve Consumer Complaints",     "Approve consumer complaint investigation outcomes", False),
    ("consumer_complaints", "close",       "Close Consumer Complaints",       "Resolve, compensate, or close consumer complaints", False),
    ("consumer_complaints", "link_recall", "Link Complaint To Recall",        "Escalate or link consumer complaints to recall workflows", False),
    ("consumer_complaints", "export",      "Export Consumer Complaints",      "Export consumer complaint and recall linkage reports", False),
...
SCOPE_AWARE_PERMISSIONS = [
    ...
    ("mps", "view", "View MPS", "View MPS plans, lines, capacity, campaigns, and what-if scenarios", True),
    ("mps", "create", "Create MPS", "Create MPS plans", False),
    ("mps", "edit", "Edit MPS", "Edit MPS plan lines and planner overrides", False),
    ("mps", "approve", "Approve MPS", "Approve MPS plans for release", False),
    ("mps", "release", "Release MPS", "Release approved MPS plans into production orders", False),
    ("mps", "calculate", "Calculate MPS", "Generate MPS from MRP and run capacity/campaign scheduling", False),
    ("mps", "simulate", "Simulate MPS", "Create MPS what-if scenarios", False),
    ("mps", "ai", "Run MPS AI", "Run and review MPS AI recommendations", False),
    ...
    ("npd", "view_all", "View All NPD", "View all NPD projects and stage gates", True),
    ("npd", "create_all", "Create All NPD", "Create NPD projects across all scopes", False),
    ("npd", "edit_all", "Edit All NPD", "Edit NPD projects and checklists across all scopes", False),
    ("npd", "approve_all", "Approve All NPD", "Approve NPD stage gates across all scopes", False),
    ("npd", "advance_all", "Advance All NPD", "Advance NPD projects across all scopes", False),
    ("npd", "pilot_all", "Manage All NPD Pilots", "Manage NPD pilot batches across all scopes", False),
    ("npd", "export_all", "Export All NPD", "Export NPD records across all scopes", False),
    ("bom", "view_all", "View All BOM", "View all BOM/formula records", True),
    ("bom", "create_all", "Create All BOM", "Create BOM/formula records across all scopes", False),
    ("bom", "edit_all", "Edit All BOM", "Edit BOM/formula records across all scopes", False),
    ("bom", "delete_all", "Delete All BOM", "Delete BOM/formula records across all scopes", False),
    ("bom", "approve_all", "Approve All BOM", "Approve BOM/formula records across all scopes", False),
    ("bom", "release_all", "Release All BOM", "Release BOM/formula records across all scopes", False),
    ("bom", "archive_all", "Archive All BOM", "Archive BOM/formula records across all scopes", False),
    ("bom", "cost_all", "Cost All BOM", "Run BOM costing across all scopes", False),
    ("bom", "ai_all", "Run All BOM AI", "Run and action BOM AI recommendations across all scopes", False),
    ("bom", "export_all", "Export All BOM", "Export BOM/formula records across all scopes", False),
    ("recipe", "view_all", "View All Recipes", "View all recipe records", True),
    ("recipe", "create_all", "Create All Recipes", "Create recipe records across all scopes", False),
    ("recipe", "edit_all", "Edit All Recipes", "Edit recipe records across all scopes", False),
    ("recipe", "delete_all", "Delete All Recipes", "Delete recipe records across all scopes", False),
    ("recipe", "approve_all", "Approve All Recipes", "Approve recipes across all scopes", False),
    ("recipe", "obsolete_all", "Obsolete All Recipes", "Obsolete recipes across all scopes", False),
    ("recipe", "export_all", "Export All Recipes", "Export recipe records across all scopes", False),
...
ROLE_DEFINITIONS = {
    ...
    "admin": {
        ...
        "permissions": [
            ...
            "consumer_complaints.delete", "consumer_complaints.approve", "consumer_complaints.close",
            "consumer_complaints.link_recall", "consumer_complaints.export",
            "npd.view", "npd.create", "npd.edit", "npd.approve", "npd.advance", "npd.pilot", "npd.export",
            "bom.view", "bom.create", "bom.edit", "bom.delete", "bom.approve", "bom.release",
    ...
    "ceo": {
        ...
        "permissions": [
            ...
            "consumer_complaints.view", "consumer_complaints.export",
            "npd.view", "npd.export",
            "bom.view", "bom.cost", "bom.export",
            "recipe.view", "recipe.export",
            "maintenance.view",
    ...
    "coo": {
        ...
        "permissions": [
            ...
            "mps.view", "mps.create", "mps.edit", "mps.approve", "mps.release",
            "mps.calculate", "mps.simulate", "mps.ai",
            ...
            "consumer_complaints.view", "consumer_complaints.create", "consumer_complaints.edit",
            "consumer_complaints.close", "consumer_complaints.link_recall", "consumer_complaints.export",
            "npd.view", "npd.create", "npd.edit", "npd.approve", "npd.advance", "npd.pilot", "npd.export",
            "bom.view", "bom.create", "bom.edit", "bom.approve", "bom.release", "bom.archive",
            "bom.cost", "bom.ai", "bom.export",
            "recipe.view", "recipe.create", "recipe.edit", "recipe.approve", "recipe.obsolete", "recipe.export",
    ...
    "production_supervisor": {
        ...
        "permissions": [
            ...
            "mps.view", "mps.edit", "mps.calculate",
    ...
    "production_manager": {
        ...
        "permissions": [
            ...
            "npd.view", "npd.create", "npd.edit", "npd.approve", "npd.advance", "npd.pilot", "npd.export",
            "bom.view", "bom.create", "bom.edit", "bom.approve", "bom.release", "bom.cost", "bom.export",
            "recipe.view", "recipe.create", "recipe.edit", "recipe.approve", "recipe.obsolete", "recipe.export",
            "planning.view_all", "planning.create_own_scope", "planning.edit_own_scope",
            "planning.calculate_own_scope", "planning.approve_own_scope",
            "production.create_own_scope", "production.edit_own_scope", "production.release_own_scope",
    ...
    "quality_manager": {
        ...
        "permissions": [
            ...
            "consumer_complaints.view", "consumer_complaints.create", "consumer_complaints.edit",
            "consumer_complaints.approve", "consumer_complaints.close",
            "consumer_complaints.link_recall", "consumer_complaints.export",
            "npd.view", "npd.approve", "npd.export",
            "bom.view", "bom.approve", "bom.release", "bom.export",
            "recipe.view", "recipe.approve", "recipe.export",
        ],
    },
    "procurement_manager": {
    ...
    "read_only_auditor": {
        ...
        "permissions": [
            ...
            "consumer_complaints.view", "consumer_complaints.export",
            "npd.view", "npd.export", "bom.view", "bom.export", "recipe.view", "recipe.export",
